...
- I'm using uv instead of poetry for package management since uv is awesome. I changed the lock file, and I also had to modify the yaml files in the .github/workflows directory (might cause issues idk)
...
- May add in Grok-4 since it has access to Twitter, and it does well on the AA-LCR (Long Context Reasoning) benchmark (see 8th chart in intelligence evaluation section of https://artificialanalysis.ai/)
...
5. These results can then be appended to the initial prompt of the main research agents

## GPT-5

- litellm and openrouter support the text completion API for openai, not the responses API

- GPT-5 chat is the model being used in openrouter (not normal gpt-5). This is an issue because I dont think gpt-5 chat allows custom reasoning levels parameters the same way the normal gpt-5 API does. (TODO: figure this out) -- normal gpt-5 needs a separate API key in openrouter??

- I only want to use the "high" setting for research agents, and maybe a high verbosity level. The argument for reasoning level is reasoning_effort, and this needs to get passed through the Metaculus forecasting tools wrapper, litellm, and then to openrouter and openai
